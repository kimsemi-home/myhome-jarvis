...
func TestStatusConnectsLearningObservationsToEvidenceArtifacts(t *testing.T) {
	...
	if status.SourceCount != 7 || status.PresentSourceCount != 2 {
...
func writeEvidencePolicy(t *testing.T, root string, redacted bool) {
  ...
  "node_kinds": ["learning_observation", "evidence_artifact", "checkpoint", "control_plane_manifest", "incident", "quality_run", "linear_write", "audit_event"],
