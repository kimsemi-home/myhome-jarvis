...
func TestRehashedUnsafeCreditOnboardingProofFails(t *testing.T) {
	...
	report.ImportTemplate.Onboarding.AmbiguousProfileRejected = false
	...
	if err := validateProofBody(tampered, "2026-07", ref); err == nil {
		t.Fatal("accepted rehashed credit proof with an ambiguous-profile guard disabled")
	}
}
