...
func TestROISummaryUsesAttributionWithoutDoubleCountingTotal(t *testing.T) {
	...
	writeFile(t, root, "data/private/codex-cost/attribution.jsonl",
		`{"at":"2026-06-19T00:00:01Z","scope":"repo","subject_key":"repo:kimsemi-home/myhome-jarvis","cost_ref":"cost_ref_test","subject_hash":"subject_test","unit_kind":"codex_tokens","amount":100,"basis":"merged_pr","evidence_refs":["docs/codex-cost-governor.md"]}`+"\n"+
			`{"at":"2026-06-19T00:00:02Z","scope":"linear_project","subject_key":"linear:KIM-133","cost_ref":"cost_ref_test","subject_hash":"subject_linear","unit_kind":"codex_tokens","amount":100,"basis":"merged_pr","evidence_refs":["docs/codex-cost-governor.md"]}`+"\n")
	...
	if summary.TotalUnits != 100 || summary.AttributedUnits != 100 ||
		summary.AttributionEntryUnits != 200 {
		t.Fatalf("summary = %#v", summary)
	}
	if summary.AttributionCostRefCount != 1 {
		t.Fatalf("cost refs = %#v", summary)
	}
	if summary.AttributionCoveragePercent != 100 {
		t.Fatalf("coverage = %#v", summary)
	}
	if rows["repo"].Status != "attributed" ||
		rows["repo"].CostUnits != 100 ||
