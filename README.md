...
- Go toolchain is pinned to 1.26.5.
...
scopes. Each connection resolves one versioned credential bundle from an
`op://` reference, maps the supported bank and card transaction contracts, and
...
credentials; if Toss rejects a cached token, it refreshes once and retries.
It never prints the CI, signed consent, resolved 1Password bundle, or raw
provider data.
