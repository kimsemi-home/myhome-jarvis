...
| Finance | `financeDashboard` normalized fixture/live domain summary. | empty, fixture-only, consented live, owner scoped, card-linked review, verified metadata. | Do not import bank/card payloads, expose account identifiers, trigger card actions, or write to finance ledgers. |
...
- Household scopes derive `selected scope`, `owner scoped`, `household scoped`,
  `summary-only`, and `empty` badges in
  `apps/flutter/lib/ui/household_scope_state.dart`.
