...
(defun validate-github-action-refs (refs)
  ...
  (dolist (ref refs)
    ...
    (dolist (input (policy-list ref :inputs))
      (require-string-value (getf input :key)
                            "GitHub action input key is required")
      (require-string-value (getf input :value)
                            "GitHub action input value is required"))))
