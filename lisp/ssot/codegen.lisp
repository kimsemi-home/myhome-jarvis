...
(defun validate-code-shape-policy (policy)
  (let ((legacy (coerce (getf policy :legacy_debt_files) 'list))
        (roots (coerce (getf policy :source_roots) 'list))
        (extensions (coerce (getf policy :extensions) 'list))
        (summary (coerce (getf policy :public_summary_fields) 'list)))
    (unless (string= (getf policy :context) "AgentCluster")
      (error "Code shape policy must belong to AgentCluster"))
    (unless (string= (getf policy :generated_artifact)
                     "generated/code_shape.generated.json")
      (error "Code shape generated artifact path is invalid"))
    (unless (= (getf policy :max_file_lines) 75)
      (error "Code shape max file lines must stay 75"))
    (unless (getf policy :public_status_redacted)
      (error "Code shape public status must stay redacted"))
    (unless (and (> (length roots) 0) (> (length extensions) 0))
      (error "Code shape roots and extensions are required"))
    (dolist (extension extensions)
      (unless (and (stringp extension)
                   (> (length extension) 1)
                   (char= (aref extension 0) #\.))
        (error "Code shape extension is invalid: ~A" extension)))
    (dolist (entry legacy)
      (let ((path (getf entry :path))
        ...
        (unless (and (stringp path) (> (length path) 0)
                     ...
                     (integerp max-lines)
                     (> max-lines 75))
          (error "Code shape legacy debt entry is invalid: ~A" path))))
    (dolist (field '("budget_regression_count" "legacy_debt_count" "ok"))
      (unless (find field summary :test #'string=)
        (error "Code shape public summary missing field: ~A" field)))
    (unless (find "mhj code-shape status"
                  (coerce (getf policy :commands) 'list)
                  :test #'string=)
      (error "Code shape status command must stay in SSOT"))
    t))
...
(defun write-generated-artifacts (root)
  ...
  (write-json-file (merge-pathnames "generated/authority.generated.json" root)
                   *authority-policy*)
  (write-json-file (merge-pathnames "generated/code_shape.generated.json" root)
                   *code-shape-policy*)
