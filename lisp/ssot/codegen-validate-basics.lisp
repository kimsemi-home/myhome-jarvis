...
(defun validate-security-policy-basics ()
  ...
  (let ((cache (getf *security-policy* :status_cache)))
    (require-true (getf cache :enabled)
                  "Security status cache must stay enabled")
    (require-private-path (getf cache :path)
                          "Security status cache must stay private")
    (require-string-equal (getf cache :mode) "history_aggregate_only"
                          "Security status cache must store aggregates only")
    (require-true (getf cache :miss_runs_full_history)
                  "Security status cache misses must run full history")
    (require-true (getf cache :current_scan_always_fresh)
                  "Security current scan must always stay fresh")
    (require-false (getf cache :raw_findings_public_allowed)
                   "Security status cache must not expose raw findings")))
